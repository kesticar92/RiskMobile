...
### 12.1) `kevin-main` — RF-K15 a RF-K19 (**implementados**)

| ID | Nombre | Comportamiento en la app |
|----|--------|--------------------------|
| **RF-K15** | Fecha de proximo seguimiento | Campo `nextFollowUpAt` en el caso. En detalle: elegir fecha o quitar. En CRM: chip **Próx. seguimiento** (orden por fecha; casos sin fecha al final) y en tarjeta `_ClientFollowUpChip` (vencido / hoy / fecha). |
| **RF-K16** | Etiquetas (tags) | `caseTags` (max. 8, normalizadas en minusculas). Detalle: chips, añadir y **Guardar etiquetas**. CRM: `Filtrar por etiqueta` (multiseleccion entre tags existentes en cartera). |
| **RF-K17** | Casos archivados | `caseArchived`. Detalle: interruptor; banner si esta archivado. CRM: `Incluir archivados` (conteo); icono de archivo en tarjeta. Por defecto los archivados no se listan. |
